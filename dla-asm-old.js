...
//            }
    ...
    console.log("rowbits=" + rowbits);
    var heapbytes = (ydim << rowbits) + (2 * 4 * pcount) + 8000;
    var heapsize = Math.pow(2, 1 + Math.ceil(Math.log(heapbytes) / Math.log(2)));
    console.log("Using heap size of " + heapsize);
//    var seed = 0xDEADBEEF;
    var seed = (Math.random() * Math.pow(2, 32)) & 0xFFFFFFFF;
    var module = dlaModule(window, { draw: draw, seed: seed, log: console.log }, new ArrayBuffer(heapsize));
